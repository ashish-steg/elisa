    ...
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext) {
        ...
        FulfillmentContact = CheckoutStep.extend({
            ...
            next: function () {
                ...
                var countryCode = this.attributes.address.attributes.countryCode;
                if(!(countryCode in countriesRequiringStateAndZip)){
                    if(!this.attributes.address.attributes.postalOrZipCode){
                        this.attributes.address.attributes.postalOrZipCode = "n/a";
                    }
                }
                ...
                if (val) {
                    // display errors:
                    var error = {"items":[]};
                    var checkMessage = function(item){ return item.message == key.substring(0, ".") + val[key]; };
                    for (var key in val) {
                        if (val.hasOwnProperty(key) && ! _.find(error.items, checkMessage)) {
                            var errorItem = {};
                            errorItem.name = key;
        ...
        var CheckoutPage = Backbone.MozuModel.extend({
            ...
            setFulfillmentContactEmail: function () {
                var fulfillmentEmail = this.get('fulfillmentInfo.fulfillmentContact.email'),
                    orderEmail = this.get('emailAddress');
            ...
            syncBillingAndCustomerEmail: function () {
                ...
                if (!customerEmail || customerEmail != billingEmail) {
            ...
            submit: function () {
                var order = this,
                    ...
                    requiresDigitalFulfillmentContact = this.get('requiresDigitalFulfillmentContact'),
                ...
                if (requiresDigitalFulfillmentContact && !this.get("fulfillmentInfo").get("fulfillmentContact").get("email")){
                    this.isSubmitting = false;
                    this.onCheckoutError({message: "Please enter a valid gift card recipient"});
                    return false;
                }
