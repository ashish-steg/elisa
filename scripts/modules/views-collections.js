...
define([
    ...
    function factory(conf) {
        ...
        function updateUi(response) {
            ...
            if(window.checkImages) window.checkImages();
        ...
        function colorSwatch(){
            ...
            $(search).each(function(){
                if(this.indexOf("facetValueFilter") > -1){
                    var filter = this.split("=")[1].split(",");
                    $(filter).each(function(){
                        if(this.toLowerCase().indexOf("tenant~color") > -1){
                        color += this.split(":")[1] +":";
                        }
                    });
                }
            ...
            if(color){
                ...
                if(color.indexOf(":") == -1){
                    $(".color_swatch."+ color).trigger("click");
                } else {
                    color = color.split(":");

                    for(var i = 0; i < color.length; i++){
                        $(".color_swatch."+ color[i]).trigger("click");
                    }
                }
            }
        }
