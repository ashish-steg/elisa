define(["modules/jquery-mozu", "underscore", "modules/backbone-mozu", "hyprlive", "modules/models-price", "modules/api"], function($, _, Backbone, Hypr, PriceModels, api) {
    ...
    Product = Backbone.MozuModel.extend({
        ...
        addToCart: function () {
            var me = this;
            var incompleteOpts = _.filter(me.attributes.options.models, function(option){
                return option.attributes.isRequired && !option.attributes.value;
            });
            var err = {message: "", code: ""};
            this.whenReady(function () {
                if(incompleteOpts.length > 0){
                    err.code = "VALIDATION_CONFLICT";
                    err.message = "Validation Error: Please finish filling out all options<br/>";
                    $(incompleteOpts).each(function(){
                        err.message += this.attributes.attributeDetail.name +"<br/>";
                    });
                    ...
                    if (!me.validate()) {
                        ...
                        api.request("GET", "/api/commerce/carts/current/items").then(function(cartItems){
                            ...
                            if(inCart !== false && inCart !== 0){
                                console.log(cartItems.items[inCart].id);
                                console.log(me.get('quantity'));
                                api.request("PUT", "/api/commerce/carts/current/items/"+ cartItems.items[inCart].id +"/"+ me.get("quantity")).then(function(item){
                                    console.log(item);
        ...
        onOptionChange: function() {
            this.isLoading(true);
            $(".mz-productoptions input, .mz-productoptions select").each(function(){
        ...
        updateConfiguration: function() {
            ...
            if (JSON.stringify(this.lastConfiguration) !== JSON.stringify(newConfiguration)) {
                ...
                this.apiConfigure({ options: newConfiguration }, { useExistingInstances: true }).then(function(){
                    this.trigger("change");
                }.bind(this));
            } else {
                this.isLoading(false);
