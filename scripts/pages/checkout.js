...
require([
    ...
    var CheckoutStepView = EditableView.extend({
        ...
        next: function () {
            ...
            me.render();
            $(".mz-messagebar").children(".mz-messagebar").html("");
            _.defer(function () {
                me.model.next();
            });
        },
        ...
        render: function () {
            ...
            this.resize();
            if(this.model.parent.attributes.fulfillmentInfo){
                if(this.model.parent.attributes.fulfillmentInfo.attributes.fulfillmentContact._stepStatus != "complete"){
                    $(window).scrollTop($("#step-shipping-address").offset().top - 100);
                } else if (this.model.parent.attributes.fulfillmentInfo._stepStatus != "complete") {
                    $(window).scrollTop($("#step-shipping-method").offset().top - 100);
                } else if(this.model.parent.attributes.fulfillmentInfo._stepStatus != "complete") {
                    $(window).scrollTop($("#step-payment-info").offset().top - 100);
                }
            } else {
                if(this.model.parent.attributes.fulfillmentContact._stepStatus != "complete"){
                    if(this.model.get("address") && !this.model.get("address").get("countryCode")){
                        this.$el.find("[data-mz-value='address.countryCode']").trigger("change");
                    }
                    $(window).scrollTop($("#step-shipping-address").offset().top - 100);
                }
            }
    ...
    var OrderSummaryView = Backbone.MozuView.extend({
        ...
        render: function(){
            ...
            $(this.model.attributes.items).each(function(index, item){
    ...
    var BillingInfoView = CheckoutStepView.extend({
        ...
        additionalEvents: {
            ...
            "change [data-mz-value='acceptsMarketing']": "updateAcceptsMarketing",
            "change #mz-payment-credit-card-number": "checkCardNumber"
        },
        ...
        checkCardNumber: function(e){
            $(e.currentTarget).val($(e.currentTarget).val().replace(/\s+/g, ''));
        },
        ...
        updateAcceptsMarketing: function(e) {
            ...
            var acceptsMarketing = $(e.currentTarget).prop('checked');
            this.model.getOrder().set('acceptsMarketing', $(e.currentTarget).prop('checked'));
            api.get("customer", user.accountId).then(function(cust){
                cust.update({"acceptsMarketing": acceptsMarketing});
            });
        },
    ...
    $(document).ready(function () {
        ...
        if(checkoutViews.steps.shippingAddress.model._stepStatus != "complete"){
            $(window).scrollTop(0);
        }
    });
});
